...
# bounds that home's startup-memory curation, primary config/gitea-instances
# carries the same Gitea instance allow-list so a secondmate resolves the same
# self-hosted forges for delivery, and primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode gitea-instances lavish-axi-host}"
