...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>|alive but unsupervised: <cause>; recover with bin/fm-control.sh <id> relaunch` - the session-start liveness sweep could not guarantee that the registered secondmate is running a real agent process, or a live endpoint's OWN home was not supervising it (a stale session lock, missing startup completion, or stale beacon).
  Investigate the reason because that secondmate is not guaranteed live.
  In the alive-but-unsupervised form the agent process is genuinely running, so recovery is an explicit `bin/fm-control.sh <id> relaunch` on that home, never a force-recreate: the endpoint-alive reading is exactly what hid the gap.
